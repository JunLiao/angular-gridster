...
angular.module("gridster").constant("gridsterConfig", {
    columns: 6,
    pushing: true,
    floating: true,
    swapping: false,
...
angular.module("gridster").controller("GridsterCtrl", [ "gridsterConfig", "$timeout", function(gridsterConfig, $timeout) {
    var gridster = this;
    ...
    this.floatItemLeft = function(item) {
        ...
        var colIndex = item.col, sizeY = item.sizeY, sizeX = item.sizeX, bestRow = null, bestColumn = null, rowIndex = item.row;
        var items;
        while (colIndex > -1) {
...
angular.module("gridster").directive("gridster", [ "$timeout", "$rootScope", "$window", function($timeout, $rootScope, $window) {
    return {
        restrict: "EAC",
        transclude: true,
        replace: true,
...
angular.module("gridster").factory("GridsterDraggable", [ "$document", "$timeout", "$window", function($document, $timeout, $window) {
    function GridsterDraggable($el, scope, gridster, item, itemOptions) {
        var elmX, elmY, elmW, elmH, mouseX = 0, mouseY = 0, lastMouseX = 0, lastMouseY = 0, mOffX = 0, mOffY = 0, minTop = 0, maxTop = 9999, minLeft = 0, realdocument = $document[0];
        ...
        function drag(event) {
            var oldRow = item.row, oldCol = item.col, hasCallback = gridster.draggable && gridster.draggable.drag, scrollSensitivity = gridster.draggable.scrollSensitivity, scrollSpeed = gridster.draggable.scrollSpeed;
...
angular.module("gridster").factory("GridsterResizable", [ function() {
    function GridsterResizable($el, scope, gridster, item, itemOptions) {
        function ResizeHandle(handleClass) {
            var hClass = handleClass;
            var elmX, elmY, elmW, elmH, mouseX = 0, mouseY = 0, lastMouseX = 0, lastMouseY = 0, mOffX = 0, mOffY = 0, minTop = 0, maxTop = 9999, minLeft = 0;
            ...
            function resizeStart(e) {
                ...
                scope.$apply(function() {
                    if (gridster.resizable && gridster.resizable.start) {
                        gridster.resizable.start(e, $el, itemOptions);
                    }
                });
            }
            function mouseMove(e) {
            ...
            function resize(e) {
                var oldRow = item.row, oldCol = item.col, oldSizeX = item.sizeX, oldSizeY = item.sizeY, hasCallback = gridster.resizable && gridster.resizable.resize;
                var col = item.col;
                ...
                if (hasCallback || isChanged) {
                    scope.$apply(function() {
                        if (hasCallback) {
                            gridster.resizable.resize(e, $el, itemOptions);
                        }
                    });
                }
            }
            function resizeStop(e) {
                ...
                scope.$apply(function() {
                    if (gridster.resizable && gridster.resizable.stop) {
                        gridster.resizable.stop(e, $el, itemOptions);
                    }
                });
            }
            var $dragHandle = null;
...
angular.module("gridster").directive("gridsterItem", [ "$parse", "GridsterDraggable", "GridsterResizable", function($parse, GridsterDraggable, GridsterResizable) {
    return {
        restrict: "EA",
        ...
        link: function(scope, $el, attrs, controllers) {
            ...
            var aspects = [ "minSizeX", "maxSizeX", "minSizeY", "maxSizeY", "sizeX", "sizeY", "row", "col", "resizable", "draggable" ], $getters = {};
